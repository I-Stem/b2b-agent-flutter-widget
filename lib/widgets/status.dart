...
class _AnimatedOpacityWidget extends StatefulWidget {
  ...
  const _AnimatedOpacityWidget({required this.agentState, required this.child});
...
class _AnimatedOpacityWidgetState extends State<_AnimatedOpacityWidget>
    ...
    switch (widget.agentState) {
      case AgentState.thinking:
        return const Duration(
          milliseconds: 500,
        ); // Faster animation for thinking
      default:
        return const Duration(
          milliseconds: 1000,
        ); // Default duration for other states
    }
  }
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder:
          (context, child) => Opacity(opacity: _getOpacity(), child: child),
      child: widget.child,
    );
  }
}
